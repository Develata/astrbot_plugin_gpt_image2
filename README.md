...
> **默认权限：管理员。** v0.4.0 起，所有 `/gptimg*` 与 `/gptedit` 指令都通过 AstrBot `PermissionType.ADMIN` 收紧为管理员可用。插件默认不再开放“所有人”权限，因为默认开放给所有人的生图插件，通常不能保护好自己的 API key，很容易被无意或恶意调用用爆额度。
>
> 插件也会把 `gpt_image2_generate` / `gpt_image2_edit` 两个 LLM Tool 的默认权限初始化为 `admin`；如果你已经在 AstrBot WebUI 中显式配置过工具权限，插件会保留你的设置。
>
> 如果你确认要开放给普通成员，请在 AstrBot 指令管理/权限管理中显式下放对应指令或工具权限，并配合下文“使用限制”的用户白名单、黑名单、群白名单与每日额度使用。勿裸奔，慎之。
...
使用限制在 LLM Tool 路径同样生效：如果是用户让 agent 使用生图工具，插件按该事件的 sender/group 做白名单与每日额度检查。v0.4.0 起，两个 LLM Tool 的默认 AstrBot 工具权限也会初始化为 `admin`，防止普通成员绕过指令权限让模型代为调用生图工具；如需开放，请在 AstrBot WebUI 中显式调整工具权限。
...
- 当前 endpoint 只要抛出错误，就继续尝试下一条 fallback endpoint，包括 DNS/连接失败、HTTP 错误、timeout 与 `5xx`。
- 不重试同一个 endpoint。
- 若最终使用了 fallback endpoint，非静默模式会在最终提示中写明实际路由到的 endpoint 与 model，例如 `fallback: 已路由到 fallback_1 / model: gpt-image-2`。
- `runtime.quiet_mode=true` 时不额外提示 fallback 路由信息，成功路径仍只发送最终成品图。
...
`access.enabled=false` 时不做插件内使用限制；但 v0.4.0 起命令层默认仍是 AstrBot 管理员权限，LLM Tool 默认工具权限也会初始化为管理员。也就是说，默认安装后只有管理员可以通过指令或工具提交生图任务；若你在 AstrBot 中把指令/工具权限下放给普通成员，才需要依赖这里的插件内白名单/黑名单/额度进一步限流。
...
- endpoint 报错：若配置了 fallback，会继续尝试下一条 endpoint；所有 endpoint 都失败时，向原会话发送合并后的脱敏错误摘要。
- 由于 v0.4.1 起按你的运行策略“所有报错都走 fallback”，timeout / `504` / `5xx` 也会切换到备用 endpoint；请只配置你确认为同一计费/幂等风险可接受的备用通道。
